...
@Factory
public class VideoDislikedFactory {
    ...
    KStream<String, Integer> likedDislikedStream(ConfiguredStreamBuilder builder) {
    	...
    	KStream<Long, Video> dislikedVideos = builder.stream("video-disliked", Consumed.with(Serdes.Long(), serdeRegistry.getSerde(Video.class)));
		...
		KStream<String, Integer> hashtagStream = dislikedVideos.flatMap(
				(key, value) -> {
